...
function FeeListView({ data }: Props) {
  ...
  return (
    <div className="py-6 grid grid-cols-1 gap-y-4 border bg-slate-50/50 rounded-xl shadow-[0px_0px_8px_#ddd_inset]">
      ...
      { data.length ? (
      <div className={`${sum > 0 ? 'text-red-600/80' : 'text-primary/70'} px-6 pb-4 md:grid grid-cols-6 place-items-center border-slate-200 text-base font-sans font-semibold uppercase tracking-widest`}>
          <div className="place-self-start">&nbsp;</div>
          <div className="col-span-5 place-self-start flex items-center justify-between"><span>NET { sum > 0 ? 'DEBT':'BALANCE'}:</span>&nbsp;&nbsp;&nbsp;<span>{data && data[0]?.currency} {sum > 0 ? sum : Math.abs(sum)}</span></div>
      </div>
      ): null }
