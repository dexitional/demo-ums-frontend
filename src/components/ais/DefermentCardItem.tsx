  ...
  return (
    <div className="p-4 md:p-6 min-h-max border border-primary/20 rounded-xl bg-slate-50/50 hover:bg-slate-100 space-y-4 md:group">
      ...
      <div className="w-full space-y-2 font-roboto">
        ...
        <div className="flex items-center space-x-4">
          ...
          <span className="text-xs text-gray-500 font-semibold tracking-wider">
            STARTS:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{" "}
            {(data?.start &&
              moment(data?.start).format("MMM DD, YYYY").toUpperCase()) ||
              "NOT SET"}
          </span>
        </div>
        <div className="flex items-center space-x-4">
          ...
          <span className="text-xs text-gray-500 font-semibold tracking-wider">
            ENDS:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{" "}
            {(data?.end &&
              moment(data?.end).format("MMM DD, YYYY").toUpperCase()) ||
              "NOT SET"}
          </span>
        </div>
        <div className="flex items-center space-x-4">
          ...
          <span className="text-xs text-gray-500 font-semibold tracking-wider">
            LETTER DATE:&nbsp;&nbsp;&nbsp;&nbsp;{" "}
            {(data?.letterDate &&
              moment(data?.letterDate).format("MMM DD, YYYY").toUpperCase()) ||
              "NOT SET"}
          </span>
        </div>
        <hr />
        ...
        <div className="flex justify-between">
          <div className="flex items-center space-x-1">
            <span className="px-2 py-0 bg-green-50 rounded border font-bold text-sm text-gray-500">
              YEAR: {Math.ceil(data?.semesterNum / 2)}
            </span>
          </div>
          <div className="flex items-center space-x-1">
            <span className="px-2 py-0 bg-green-50 rounded border font-bold text-sm text-gray-500">
              SEMESTER: &nbsp;{data?.semesterNum % 2 == 0 ? 2 : 1}
            </span>
          </div>
        </div>
      </div>
      <div className="flex flex-col space-y-1">
        ...
        <div className="px-3 py-2 opacity-80 md:opacity-100 flex rounded-md border bg-white items-center justify-between space-x-2 group">
          ...
          <div className="hidden md:flex items-center justify-center space-x-3 text-center">
            <span
              className={`bg-green-700/60 py-0.5 px-2 rounded flex items-center space-x-1.5 text-sm text-white font-semibold`}
            >
              {data?.status}
            </span>
          </div>
