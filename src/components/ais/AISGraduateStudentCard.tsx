...
function AISGraduateStudentCard({ title,data, index }: Props) {
  ...
  const generateSheet = async () => {
    ...
    if (ok) {
      const dm = await Service.fetchStudentTranscripts({ data: data?.map( r => r.student?.indexno), key: index });
  ...
  return (
    ...
    <h1 className="w-full text-sm font-bold font-roboto tracking-wider text-primary-dark/60 flex flex-col md:flex-row justify-between">
      ...
      <div className="flex items-center space-x-2">
        <span className="px-3 py-1 rounded bg-primary/70 text-xs text-white font-bold flex items-center">{data?.length} </span>
