...
function EvaluationCardItem({ data }: Props) {
    ...
    <div className="p-4 md:p-6 min-h-max border border-primary/20 rounded-xl bg-slate-50/50 hover:bg-slate-100 space-y-4 md:group">
      ...
      <div className="space-y-2 font-roboto">
        ...
        <div className="flex items-center space-x-4">
          ...
          <span className="px-2 py-0 bg-green-50 rounded border font-bold text-xs text-gray-500">
      ...
      <div className="flex flex-col space-y-1">
        ...
        <div className="px-3 py-2 opacity-80 md:opacity-100 flex rounded-md border bg-white items-center space-x-2 group">
          <Link
            to={`/print/evaluation/${encodeURIComponent(data?.student?.id)}`}
          ...
          {["ais techlead", "ais admin"].includes(
            ...
            <>
              {/* <Form
                method="post"
                action={`${encodeURIComponent(data?.student?.id)}/destroy`}
                onSubmit={(e) => {
